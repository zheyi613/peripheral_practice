...
void usart3_default_init(void)
{
        ...
        gpio_set_mode(GPIOD, USART3_TX_PIN, GPIO_MODE_ALTFN);
        // Set PD9 (Rx) mode to alternate function.
        gpio_set_mode(GPIOD, USART3_RX_PIN, GPIO_MODE_ALTFN);
        ...
        MODIFY_REG(USART3->CR1, USART_TE_MASK | USART_RE_MASK, (1 << USART_TE_POS) | (1 << USART_RE_POS));
        // Set baud rate
        USART3->BRR = (PERIPH_CLK + (115200 / 2U)) / 115200;
        // enable USART3
        MODIFY_REG(USART3->CR1, USART_UE_MASK, 1);
}
