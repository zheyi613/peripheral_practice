...
static void i2c_set_ownAddress1(I2C_reg_t *I2Cx, uint32_t address);
...
static void i2c_set_ownAddress1(I2C_reg_t *I2Cx, uint32_t address)
{
	address = (address & ~0x7FU) ? (address | I2C_OAR1_OA1MODE) : (address << 1);
	MODIFY_REG(I2Cx->OAR1, I2C_OAR1_OA1 | I2C_OAR1_OA1MODE, address);
}
...
void i2c_transmit(I2C_reg_t *I2Cx, const uint16_t address, const uint8_t reg, const uint8_t size, uint8_t *data)
{
	...
	I2Cx->CR2 &= 0U;
	// Set slave address
	I2Cx->CR2 |= (address & ~0x7FU) ? (address | I2C_CR2_ADD10) : (address << 1);
	// Set transfer size
